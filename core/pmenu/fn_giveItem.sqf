...
if((lbCurSel 2005) == -1) exitWith {hint "Du hast kein Item ausgewählt.";ctrlShow[2002,true];};
...
if(!([_val] call TON_fnc_isnumber)) exitWith {hint "Du hast keine richtige Nummer eingegeben!";ctrlShow[2002,true];};
if(parseNumber(_val) <= 0) exitWith {hint "Du hast keine richtige Anzahl eingegeben!";ctrlShow[2002,true];};
if(isNil "_unit") exitWith {ctrlShow[2001,true]; hint "Der Spieler ist nicht in der nähe!";};
if(!([false,_item,(parseNumber _val)] call life_fnc_handleInv)) exitWith {hint "Du hast nicht die Anzahl an Items!";ctrlShow[2002,true];};
[[_unit,_val,_item,player],"life_fnc_receiveItem",_unit,false] spawn life_fnc_MP;
_type = [_item,0] call life_fnc_varHandle;
_type = [_type] call life_fnc_varToStr;
hint format["Du hast %2 %1 %2 gegeben!",_unit getVariable["realname",name _unit],_val,_type];
